...
def order(starting_floor, queue):
    """Это первый более или менее конечный вариант решения, который даёт правильный
    ответтв половине тестовых случаев. Из-за большого количества циклов (в частности
    циклов while и условных операторов, скрипт долго работает на больших тестах.
    TODO: нужно искать другое решение.)"""
    stops = []
    current_floor = starting_floor

    # Определим первоначальное направление движения лифта. Лифт должен двигаться
    # в сторону первого в очереди пассажира.
    p0 = queue[0]
    direction = 1 if p0["from"] >= current_floor else -1  # +1 если
    # направление вверх, -1 если направление вниз
    ...
    while queue or passengers_on_current_floor or passengers_on_elevator:
        # Лифт начинает двигаться от позиции starting_floor в сторону первого в
        # очереди пассажира.

        # Проверяем если есть пассажиры в лифте, то определяем следующий этаж, на
        # котором нужно сделать остановку, чтобы выпустить пассажира(ов).
        if passengers_on_elevator:
            for p in passengers_on_elevator:
                if p["to"] == current_floor:
                    if stops[-1] != current_floor:
                        stops.append(current_floor)
                    passengers_on_elevator.remove(p)

        # Проверяем наличие пассажиров на текущем этаже.
        # for current_floor in floors:
        passengers_on_current_floor = [p for p in queue if p["from"] == current_floor]

        # Если на текущем этаже есть пассажиры, то для каждого из них проверяем:
        # если направление, в котором ему нужно двигаться совпадает с текущим
        # направлением движением лифта - direction, то делаем остановку(!)
        # и забираем этого пассажира в лифт (passengers_on_elevator) и исключаем
        # его из очереди.
        if passengers_on_current_floor:
            while passengers_on_current_floor:
                p = passengers_on_current_floor.pop()
                if get_current_passenger_direction(p) == direction:
        ...
        # Если лифт дошёл до крайнего этажа в очереди, то разворачиваемся в
        # обратном направлении.
        if (
            current_floor == max_floor_in_queue
            and direction == 1
            or current_floor == min_floor_in_queue
            and direction == -1
        ):
            direction = -direction
